...
def show_help_manual():
    """显示操作手册"""
    st.markdown("# 📚 智能APS系统操作手册")
    st.markdown("### 您的智能计划与排程助手")
    st.markdown("---")

    # 快速导航
    st.markdown("### 🚀 快速导航")
    quick_nav_cols = st.columns(4)
    ...
    # 系统概述
    with st.expander("🎯 系统概述", expanded=True):
        st.markdown("""
        ## 欢迎使用安素雅智能APS系统

        本系统是融合了最新AI技术的新一代智能计划与排程解决方案。

        ### 主要功能模块：
        - **需求管理**：管理客户订单和需求预测
        - **智能预测**：基于历史数据的多场景预测
        - **产销协同**：S&OP计划制定与优化
        - **生产计划**：MPS主生产计划和MRP物料需求计划
        - **车间调度**：详细的车间作业排程
        - **物流管理**：仓储网络分析和发运计划
        - **实时监控**：智能指挥中心和KPI仪表板

        ### 快速开始：
        1. 点击左侧"生成数据"按钮创建示例数据
        2. 选择需要的功能模块
        3. 根据业务需求配置参数
        4. 执行相应的计划或分析功能

        ### 模块间数据传递关系：
        1. 需求预测链路数据流： 销售历史 → 智能预测 → 产销规划 → 主生产计划。智能预测输出forecast_results（预测数据），产销规划接收预测数据，结合市场情报生成consensus_plan（协调计划），主生产计划使用协调后的需求数据生成production_plan（MPS数据）。
        2. 订单执行链路数据流： 客户订单 → 智能分单 → 车间排程 → 生产执行。需求管理维护orders（订单数据），智能分单为每个订单分配工厂分配字段，车间排程读取已分配的订单，生成workshop_schedule（详细排程）。
        3. 物料保障链路数据流： MPS → MRP → 采购计划 / 生产备料。主生产计划的production_plan作为MRP的输入，物料计划结合BOM计算生成material_requirements（物料需求），生产备料根据workshop_schedule和material_requirements生成备料计划。
        4. 物流配送链路数据流： 完工订单 → 发运计划 → 配送执行。车间排程完成的订单进入待发运状态，发运计划整合多个订单，优化shipping_plan（发运批次），仓网分析提供最优配送路径。

        ### 模块协作场景示例：
        1. 新订单处理流程：需求管理录入新订单，智能分单根据工厂能力和成本分配订单，车间排程安排具体生产时间和设备，物料计划计算所需物料，生产备料检查物料可用性，生产执行按计划生产，发运计划安排配送。
        2. 紧急订单插单：需求管理标记紧急订单，智能指挥中心发出预警，车间排程使用CR或EDD规则重排，生产备料优先配置物料，数字孪生模拟产能影响。
        3. 供应链中断应对：智能指挥中心检测到物料短缺预警，物料计划识别受影响的生产计划，智能分单重新分配到有库存的工厂，主生产计划调整生产计划，AI助手提供替代方案建议。
        """)

    # 基础操作
    with st.expander("🔧 基础操作指南"):
        st.markdown("""
        ## 一、系统导航

        ### 1. 侧边栏控制
        - **功能模块选择**：通过单选按钮选择要使用的功能
        - **生成数据**：创建示例数据用于测试和演示
        - **智能排程**：一键执行订单的智能排程
        - **更新孪生**：刷新数字孪生体的设备状态
        - **风险分析**：生成供应链风险热力图
    ...
    # 核心功能模块
    with st.expander("📊 需求管理模块"):
        st.markdown("""
        ## 需求管理使用指南

        ### 功能说明
        管理客户订单，查看需求分布，进行订单筛选和分析。

        ### 操作步骤
        1. **查看订单列表**
           - 显示所有客户订单信息
           - 包括订单编号、产品、数量、交期、优先级等
    ...
    with st.expander("🔮 智能预测模块"):
        st.markdown("""
        ## 智能预测使用指南

        ### 功能说明
        基于历史销售数据，使用多种算法模型进行需求预测。

        ### 操作步骤
        1. **设置预测参数**
           - 预测周期：选择预测天数（7-365天）
           - 预测场景：normal(常规)/optimistic(乐观)/pessimistic(悲观)等
           - 置信水平：设置预测置信区间（80%-99%）
    ...
    with st.expander("📈 产销规划(S&OP)模块"):
        st.markdown("""
        ## 产销规划使用指南

        ### 功能说明
        协调销售预测与生产能力，制定平衡的产销计划。
    ...
    with st.expander("🎯 智能分单模块"):
        st.markdown("""
        ## 智能分单使用指南

        ### 功能说明
        智能决定订单在哪个工厂生产，优化产能利用和成本。
    ...
    with st.expander("📋 主生产计划(MPS)模块"):
        st.markdown("""
        ## 主生产计划使用指南

        ### 功能说明
        制定中期生产计划，平衡需求预测与产能约束。

        ### 参数配置
        1. **计划期间**：4周/8周/12周/16周
        2. **批量规则**：
           - 固定批量：固定生产批量
           - 经济批量：考虑换产成本
           - 最小批量：满足最小生产量
    ...
    with st.expander("❓ 常见问题解答"):
        ...
        **A**:
        ...
        ### Q3: 系统显示"请先生成示例数据"怎么办？
        **A**: 点击左侧边栏的"生成数据"按钮，系统会自动创建示例数据。
    ...
    with st.expander("📞 技术支持"):
        ...
        ### 系统版本
        - 当前版本：安素雅智能APS系统 v1.0
        - 更新日期：2024年

        ### 联系方式
        - 技术支持邮箱：support@aps-system.com
        - 服务热线：400-XXX-XXXX
        - 在线文档：docs.aps-system.com
    ...
    st.markdown("---")
    st.info("💡 提示：您可以随时点击右上角的帮助按钮查看此操作手册。")
...
def main():
    ...
    with col1:
        st.markdown('<div class="header">安素雅智能APS系统</div>', unsafe_allow_html=True)
        st.markdown("**新一代智能排程解决方案**")

    with col3:
        # 帮助按钮
        if st.button("📚 帮助", key="help_button", help="查看操作手册"):
    ...
    if not st.session_state.get('show_help', False):
        ...
        with col1:
            if st.button("🔄 生成数据", help="创建模拟数据", use_container_width=True, key="generate_data_btn"):
                load_sample_data()
                st.success("示例数据已生成!")

            if st.button("🚀 智能排程", help="运行优化算法", use_container_width=True, key="scheduling_btn"):
                if st.session_state.orders.empty:
                    st.warning("请先生成示例数据!")
                else:
        ...
        with col2:
            if st.button("📊 更新孪生", help="刷新设备状态", use_container_width=True, key="update_twin_btn"):
                if st.session_state.resources.empty:
                    st.warning("请先生成示例数据!")
                else:
            ...
            if st.button("🌍 风险分析", help="生成风险热力图", use_container_width=True, key="risk_analysis_btn"):
